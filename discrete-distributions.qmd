...
上で作った棒グラフの縦軸は回数である。これを分布として表現するために縦軸を割合 (proportion) に変更したい。そのために、`after_stat(prop)`をy軸の変数として指定して、以下のようにする。`group = 1` も必要なので注意されたい。
...
`group = 1` も必要なので注意されたい。これを忘れると、各カテゴリ内での割合が計算され、すべての棒の高さが1になってしまう。
