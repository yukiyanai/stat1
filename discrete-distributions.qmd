...
今回新たに必要になるパッケージをインストールする（Consoleに直接入力するように）。
```{r}
#| eval: false
install.packages("scales")
```

利用するパッケージを読み込む。特に、**tidyverse**パッケージを忘れずに読み込もう。
```{r}
#| message: false
library(tidyverse)
library(scales)
...
::: callout-tip
`|>` はパイプ演算子 (pipe operator)と呼ばれるもので、cmd + shift + m (macOS) または ctrl + shift + m で入力する。 
:::
...
上で作った棒グラフの縦軸は回数である。これを分布として表現するために縦軸を割合 (proportion) に変更したい。そのために、`after_stat(prop)`をy軸の変数として指定して、以下のようにする。

```{r}
bar_coin_dist <- ggplot(D, aes(x = res, y = after_stat(prop), group = 1)) +
  geom_bar(color = "black", width = 0.3) +
  labs(x = "結果", y = "割合")
plot(bar_coin_dist)
```

`group = 1` も必要なので注意されたい。これを忘れると、各カテゴリ内での割合が計算され、すべての棒の高さが1になってしまう。`y = after_stat(prop)` も `group = 1`も`aes()`の内側に書く。


縦軸をパーセント表記に変えたいときは**scales**パッケージの`scales::label_percent`が使える。
次のようにする。
```{r}
bar_coin_pct <- ggplot(D, aes(x = res, y = after_stat(prop), group = 1)) +
  geom_bar(color = "black", width = 0.3) +
  labs(x = "結果", y = "%") +
...
hist_die <- ggplot(D2, aes(x = res, y = after_stat(prop), group = 1)) +
...
hist_die_unfair <- ggplot(D3, aes(x = res, y = after_stat(prop), group = 1)) +
  geom_bar(color = "black", width = 0.5) +
  labs(x = "出た目", y = "割合") +
  scale_x_continuous(breaks = 1 : 6,
                     minor_breaks = NULL)
